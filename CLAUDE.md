...
The app targets **Expo SDK 57** (React 19.2, React Native 0.86). Peer resolution is strict — there is deliberately no `.npmrc`/`legacy-peer-deps`; if `npm install` hits an ERESOLVE, fix the offending dependency rather than reintroducing the flag.
...
**Theming.** A single source of truth in `constants/Theme.ts` (`LightTheme`/`DarkTheme`) feeds `constants/Colors.ts`. Light/dark comes from `hooks/useColorScheme.ts` (`.web` variant for web), which resolves the system scheme *through the user's dark-mode setting* (`"auto"` follows the system, `"on"`/`"off"` force a scheme). That setting lives in `hooks/ThemeModeContext.tsx`: the provider wraps `RootNavigator` in `app/_layout.tsx` (it must — every themed component resolves through it), persists changes via `storage.ts`, and withholds children until the stored preference loads so a forced scheme never flashes the system one on cold start. Trees without a provider (tests render screens directly) default to follow-the-system. Because the rendered scheme can disagree with the system's, the root `StatusBar` style is derived from the resolved scheme — don't switch it back to `"auto"`. Every screen's root is `components/GradientBackground.tsx` — a soft themed backdrop gradient (`BackgroundGradients` in `Theme.ts`) — so use it instead of a background-colored root `ThemedView` and keep the views inside it transparent: a default `ThemedView` paints a flat background-colored block that stands out against the gradient (this is what "white boxes" on a screen mean). `colors.card` is a translucent dark tint so cards read slightly darker than whichever gradient stop they sit on — never nest card-on-card, the tints stack into darker patches (StatCard's inner views are transparent for exactly this reason). Chrome that floats over content (the breathing screen's close chip) uses the near-opaque `FloatingSurface` instead. Cards use 16px padding app-wide (StatCard, settings rows, progress/summary cards). Always style through the theme, not hardcoded hex:
- Inside components, get colors via `const { colors } = useTheme()` (from `expo-router/react-navigation` — **never `@react-navigation/*`**: expo-router forked those primitives in SDK 56, the packages aren't installed, and expo-doctor flags mixing them; `ThemeProvider`, theme types, `useFocusEffect`, and `BottomTabBarProps` all come from the `expo-router/react-navigation` / `expo-router/js-tabs` entry points, and jest mocks must target those same specifiers) and reference `colors.primary`, `colors.card`, `colors.border`, etc. Theme colors are typed `ColorValue`, not `string` — accept `ColorValue` in props that receive them (as `ThemedText`/`ThemedView` do).
...
- Wrap every `fontSize`/`lineHeight` (and text-derived `minHeight`s) in `scaleFont()` from `constants/Theme.ts`: sizes are authored against a 390pt-wide screen and scale with device width, clamped to 0.85–1.15. The factor is read once at startup — rotation/resize doesn't re-scale.
...
- `utils.ts` (repo root) holds `getSessionsThisWeek`/`countSessionsThisWeek` — the one definition of the "this week" window (Sunday→Sunday), shared so Progress and Summary always agree. Compute any new week-based stat through it.
...
- `loadSettings()` → typed `{ technique, duration, visualization, isSoundEnabled, isVibrationEnabled, voice, darkMode }` with defaults (used by breathing, index, settings, and the theme-mode provider).
...
| `darkMode` | string: `"auto"` (default) / `"on"` / `"off"` — written by `ThemeModeContext`, not the settings screen directly |
...
Two environment invariants: `@react-native/jest-preset` is a devDependency because `jest-expo` 57 requires it as a peer (removing it makes every suite fail at config time), and `tsconfig.json` sets `"types": ["jest"]` because TypeScript 6 no longer auto-includes `node_modules/@types` here — without it, `jest`/`it`/`expect` are type errors in every test file (the tests still run; only `tsc` breaks).
